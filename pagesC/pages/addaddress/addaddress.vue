<template>
	<view class="add-address">
		...
		<view class="list-box" v-else>
			<view class="item" v-for="(item,index) in addressArr" :key="index">
				<view class="left">
					<view class="top">
				...
				<image @click="pageToShippingAddress2(item)" class="right" src="/pagesC/static/imgs/modify.png" mode=""></image>
			</view>
		</view>
	...
	export default {
		...
		methods:{
			...
			pageToShippingAddress(){
				uni.redirectTo({
					url:'/pagesC/pages/shippingaddress/shippingaddress'
				})
			},
			pageToShippingAddress2(item){
				uni.redirectTo({
					url:'/pagesC/pages/shippingaddress/shippingaddress?item=' + JSON.stringify(item)
				})
			}
		},
		onLoad( options ) {
